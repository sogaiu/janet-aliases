...
(comment import ./walk-dir :prefix "")
(defn wd/path-join
  [& parts]
  (string/join parts
               (dyn :path-fs-sep
...
(defn import-node-to-table
  [node]
  (match node
    [:tuple span [:symbol _ "import"] [:whitespace _ _] [:symbol _ path]
     & rest]
    (do
      (def tbl @{:_import-path path :_span span})
      (def n-elts (length rest))
      (when (pos? n-elts)
        (var idx 0)
        (while (< idx n-elts)
          (assertf (match (get rest idx)
                     [:whitespace _ _]
                     true)
                  "Unexpected node for whitespace: %p" (get rest idx))
          #
          (++ idx)
          (def name (match (get rest idx)
                      [:keyword _ kwd]
                      (keyword (string/slice kwd 1))))
          (assertf name "Unexpected node for name: %p" (get rest idx))
          #
          (++ idx)
          (assertf (match (get rest idx)
                     [:whitespace _ _]
          ...
          #
          (++ idx)
          (def value (match (get rest idx)
                       [_ _ val]
                       val))
          (assertf value "Unexpected node for value: %p" (get rest idx))
          #
          (put tbl name value)
          (++ idx))
        (assertf (= idx n-elts) "Unexpected index: %p" idx))
      #
      tbl)))
...
(defn main
  ...
  (each fp file-paths
    (def src (slurp fp))
    (when (not (empty? src))
      (def imports (collect-imports src))
      (if-not imports
        ...
        (let [i-tables
          ...
          (array/concat tables
                        (map |(when $ (put $ :_found-in fp))
                             i-tables))))))
  #
  (def grouped (group-by |(get $ :_import-path) tables))
  #
  (each import-path (sort (keys grouped))
    (def data (get grouped import-path))
    (def results (->> data
                      (filter |(get $ :as))
                      (sort-by |(get $ :as))))
    #
...
(comment
  ...
  # project root
  (def pr "/home/user/src/freja")

  # XXX: note that JANET_PATH can now contain multiple dirs
  # JANET_PATH
  (def jp "/home/user/.local/lib/janet")
